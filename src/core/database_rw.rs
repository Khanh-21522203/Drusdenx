...
impl ReadDatabase {
    ...
    pub fn from_database(db: &Database) -> Self {
        let c = &db.0.components;
        ReadDatabase {
            reader_pool: c.reader_pool.clone(),
            query_cache: c.cache.clone(),
            query_executor: c.executor.clone(),
            query_parser: c.parser.clone(),
        }
    }
...
impl WriteDatabase {
    ...
    pub fn from_database(db: &Database) -> Self {
        WriteDatabase {
            writer: db.0.components.writer.clone(),
        }
    }
...
impl MasterSlaveDatabase {
    pub fn new(config: Config, schema: SchemaWithAnalyzer, read_replicas: usize) -> Result<Self> {
        // Create master database using the new facade entry point
        let master = Arc::new(Database::open(schema, config)?);
